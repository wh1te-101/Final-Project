...
sns.lineplot(data=sample_data, x='time', y='volume', hue='direction', ci=None)
...
sns.barplot(x='重要性', y='特征', data=rf_feature_importance.head(20))
...
# 10. 模型评估
def evaluate_model(y_true, y_pred, model_name, model_type='RF'):
    """评估模型并生成可视化"""
    metrics = {
    ...
    # 只在保存时添加标题
    save_plot(plt, f'{model_name}_Model/prediction_comparison.png',
              f'{model_name}模型预测对比' if model_type == 'RF' else 'ARIMA模型预测对比')

    return metrics


# 10.1 评估随机森林
rf_metrics = evaluate_model(y_test, rf_model.predict(X_test), 'Random_Forest', 'RF')

# 10.2 评估ARIMA - 确保预测长度匹配
arima_pred_full = arima_model_fit.get_forecast(steps=len(arima_data)).predicted_mean
test_start_idx = len(arima_data) - len(y_test)
arima_pred = arima_pred_full[test_start_idx:]

# 确保长度一致
min_length = min(len(y_test), len(arima_pred))
y_test_adj = y_test[:min_length]
arima_pred_adj = arima_pred[:min_length]

arima_metrics = evaluate_model(y_test_adj, arima_pred_adj, 'ARIMA', 'ARIMA')
